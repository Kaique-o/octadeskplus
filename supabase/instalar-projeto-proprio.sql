...
-- ===== migrations/20260924000010_octaplus_integracoes_externas.sql =====
-- =====================================================================
...
-- ===== migrations/20260924000011_octaplus_endereco_cobranca.sql =====
-- =====================================================================
-- Octadesk Plus — endereço de cobrança da empresa (Configurações › Empresa, Owner).
-- Guardado em empresas.endereco_cobranca (jsonb): {cep, logradouro, numero, complemento, bairro, cidade, uf}.
-- salvar_empresa: p.endereco_cobranca grava (CEP só dígitos, UF maiúscula e válida); ausente mantém; {} ou null limpa.
-- =====================================================================

alter table octaplus.empresas add column endereco_cobranca jsonb;

create or replace function octaplus.normalizar_endereco(p jsonb) returns jsonb
language plpgsql immutable as $$
declare cep text := nullif(regexp_replace(coalesce(p ->> 'cep', ''), '\D', '', 'g'), ''); uf text := nullif(upper(btrim(coalesce(p ->> 'uf', ''))), '');
        r jsonb;
begin
  if p is null or jsonb_typeof(p) <> 'object' then return null; end if;
  if cep is not null and length(cep) <> 8 then raise exception 'cep_invalido'; end if;
  if uf is not null and uf not in ('AC','AL','AP','AM','BA','CE','DF','ES','GO','MA','MT','MS','MG','PA','PB','PR','PE','PI','RJ','RN','RS','RO','RR','SC','SP','SE','TO') then
    raise exception 'uf_invalida';
  end if;
  r := jsonb_strip_nulls(jsonb_build_object(
    'cep', cep, 'uf', uf,
    'logradouro', nullif(btrim(p ->> 'logradouro'), ''), 'numero', nullif(btrim(p ->> 'numero'), ''),
    'complemento', nullif(btrim(p ->> 'complemento'), ''), 'bairro', nullif(btrim(p ->> 'bairro'), ''),
    'cidade', nullif(btrim(p ->> 'cidade'), '')));
...
declare
  ...
  v_fuso text := nullif(btrim(coalesce(p ->> 'fuso', '')), '');
  v_logo text := nullif(btrim(coalesce(p ->> 'logo', '')), '');
  v_end jsonb := case when p ? 'endereco_cobranca' then octaplus.normalizar_endereco(p -> 'endereco_cobranca') end;
begin
  if v_fuso is not null and not exists (select 1 from pg_timezone_names where name = v_fuso) then
    raise exception 'fuso_invalido';
  end if;
  ...
  if eid is null then
    if not e_dono() then raise exception 'sem_permissao'; end if;
    if coalesce(btrim(p #>> '{master,email}'), '') = '' then raise exception 'master_obrigatorio'; end if;
    insert into empresas (nome, cnpj, telefone, site, logo, endereco_cobranca)
    values (btrim(p ->> 'nome'), v_cnpj, nullif(btrim(p ->> 'telefone'), ''), nullif(btrim(p ->> 'site'), ''), v_logo, v_end)
    returning id into eid;
    insert into configuracao (empresa_id, fuso) values (eid, coalesce(v_fuso, 'America/Sao_Paulo'));
    insert into integracao_octadesk (empresa_id) values (eid);
    perform criar_perfis_padrao(eid);
  ...
  update empresas set
    ...
    site     = case when p ? 'site' then nullif(btrim(p ->> 'site'), '') else site end,
    logo     = case when p ? 'logo' then v_logo else logo end,
    endereco_cobranca = case when p ? 'endereco_cobranca' then v_end else endereco_cobranca end
  where id = eid;
  if v_fuso is not null then
    update configuracao set fuso = v_fuso, atualizado_em = now() where empresa_id = eid;
  end if;
...
drop function octaplus.listar_empresas();
create function octaplus.listar_empresas()
returns table (id uuid, nome text, ativa boolean, cnpj text, telefone text, site text, fuso text, logo text, endereco_cobranca jsonb, criada_em timestamptz,
               perfil text, perfil_tipo text, permissoes jsonb)
language sql stable security definer set search_path = octaplus, public as $$
  select e.id, e.nome, e.ativa, e.cnpj, e.telefone, e.site, c.fuso, e.logo, e.endereco_cobranca, e.criada_em, 'Owner', 'owner',
         (select jsonb_object_agg(a, 'editar') from unnest(areas()) a)
  from empresas e left join configuracao c on c.empresa_id = e.id where e_dono()
  union all
  select e.id, e.nome, e.ativa, e.cnpj, e.telefone, e.site, c.fuso, e.logo, e.endereco_cobranca, e.criada_em, pf.nome,
         case when pf.fixo then 'master' else 'comum' end, pf.permissoes
  from empresas e join membros m on m.empresa_id = e.id join perfis pf on pf.id = m.perfil_id
  left join configuracao c on c.empresa_id = e.id
  where not e_dono() and m.user_id = auth.uid() and m.ativo and e.ativa
...
revoke execute on function octaplus.listar_empresas() from public, anon;
grant execute on function octaplus.listar_empresas() to authenticated;
revoke execute on function octaplus.normalizar_endereco(jsonb) from public, anon;
grant execute on function octaplus.normalizar_endereco(jsonb) to authenticated;
