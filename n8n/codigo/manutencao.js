...
// Entrada: um item { c } por empresa ativa = octaplus.credenciais_octadesk(). Roda a cada minuto, mas só
// trabalha quando precisa:
...
// Saída: um item { empresa_id, catalogo?, jwt? } por empresa com algo a gravar (octaplus.gravar_manutencao).

const http = this.helpers;

async function manter(c) {
  const saida = {};

  const publica = (caminho, qs) => http.httpRequest({
  ...
  if (precisaToken) {
    try {
      const r = await http.httpRequest({
...
// uma empresa por vez: a falha de uma não atrapalha as outras
const resultados = [];
for (const item of $input.all()) {
  const c = item.json.c || {};
  const saida = await manter(c);
  if (Object.keys(saida).length) resultados.push({ json: { empresa_id: c.empresa_id, ...saida } });
}
return resultados;
