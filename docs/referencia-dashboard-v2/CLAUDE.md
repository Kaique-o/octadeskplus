...
`index.html` da **raiz** → `src/app/main.jsx` → `src/app/App.jsx`. É o único HTML processado pelo build.

`src/` está organizado **por módulo**, não por tipo de arquivo. A regra e a direção permitida da dependência:

```
app  →  modulos  →  shared
plataforma  →  (pode ler qualquer camada)
```

- **`src/app/`** — o que amarra a aplicação: `App.jsx` (rotas), `main.jsx`, `index.css`, `AppShell.jsx` (casca + troca de sidebar por área), `ProtectedRoute.jsx`, `area-navegacao.js`, `titulo-pagina.js`, `telas.js`, e `paginas/` com as telas de todo mundo (Login, Configurações, Perfil). É a única camada que pode importar de `modulos/`.
- **`src/shared/`** — o que qualquer módulo usa e que **não conhece módulo nenhum**: `ui/` (KpiCard, FilterBar, Pagination, SortableTh, DrilldownModal/Chips, gráficos Chart.js), `layout/` (SidebarRodape, SyncStatusButton, NotificacoesButton), `context/` (AuthProvider, PermissionsProvider), `data/` (data-access.js, supabase-client.js), `hooks/` (useServerFilters, useDrilldown, useSkylerContext, useSyncStatus), `lib/` (xlsx, pptx, ooxml-zip, theme-color), `tours/tour.js` (o motor do guia). Importar de `modulos/` aqui é inversão de dependência — se precisar, o dado sobe por parâmetro.
- **`src/modulos/<nome>/`** — um módulo de negócio inteiro numa pasta: `nav.js` (o menu dele), `paginas/`, `tours/`, `lib/` e `componentes/` (inclusive a sidebar própria). Hoje: `compras/`, `skytech/` (o hub), `rh/`, `expedicao/`, `trocas/`, `marketing/` e `ti/`. **Módulo novo = pasta nova**; o único ponto fora dela são as rotas em `app/App.jsx`, o prefixo em `app/area-navegacao.js` e o nome em `APP_POR_AREA` (`app/titulo-pagina.js`).
- **`src/plataforma/`** — ferramenta transversal que observa o app inteiro: `debug/` (painel, provider, variações, apresentação) e `skyler/` (o chat). Pode ler de qualquer camada, de propósito: o painel de debug inspeciona rota, `NAV` e `CATALOGO`.

A lista de módulos do grupo (Compras, RH, Marketing, Trocas, Expedição, Vendas, TI, Finanças) mora em `src/modulos/skytech/modulos.js` e alimenta tanto o modal do hambúrguer quanto a tela `/hub/modulos`. Módulo com `to` é rota interna; com `url` é app externo e sai por `<a>` — com `to` o React Router trataria a URL como rota deste app e daria 404.

**Contrato de três pontas:** ao adicionar/renomear uma tela, os três precisam bater — a rota em `app/App.jsx`, o `to` em `NAV` (`modulos/<nome>/nav.js`) e a `chave` em `CATALOGO` (`shared/context/PermissionsProvider.jsx`). Cada entrada de `graficos` no `CATALOGO` (`{chave, label, tipo}`) corresponde a uma chamada `podeVerGrafico('<modulo>', '<grafico>')` dentro da página, aos mesmos nomes na tabela `permissoes_acesso` do Supabase, e é o que o editor de permissões lista.

> `CATALOGO` ainda é uma lista única dentro de `shared/context/` e hoje só descreve o Compras. Quando o segundo módulo ganhar permissão própria, ele deve passar a ser montado a partir de cada `modulos/<nome>/`, senão `shared/` volta a conhecer módulo.

### RH, Expedição, Trocas, Marketing, TI e Financeiro — telas prontas, backend pendente

`src/modulos/rh/` (Home, Colaboradores, Organograma, Jornadas, Recrutamento, Treinamentos) `src/modulos/expedicao/` (Home, Pedidos, Separação, Rastreios, Ocorrências, Transportadoras) `src/modulos/trocas/` (nove telas, na ordem do fluxo: Home, Solicitações, Triagem, Recebimentos, Análises, Aprovações, Reenvios, Créditos, Ocorrências) `src/modulos/marketing/` (Home, Campanhas, Conteúdos, Biblioteca) `src/modulos/ti/` (Home, Ativos, Sistemas, Infraestrutura, Projetos, Manuais) e `src/modulos/financeiro/` (nove telas: Home, Contas a Pagar, Contas a Receber, Fluxo de Caixa, Conciliação, Movimentações, Cobranças, Centros de Custo, Orçamentos) têm as telas **sem nenhuma RPC por trás**: não existe tabela desses domínios no Supabase nem o `dataAccess.<modulo>` correspondente.

> **Financeiro é o mais delicado de todos**: as telas mostram saldo em conta, contas a pagar, inadimplência, conciliação bancária e orçamento por área. **Não há integração bancária, não há ERP, não há coleta de nada** — um "Saldo disponível: R$ 284,3 mil" ali não é o saldo de ninguém. Todo botão que moveria dinheiro (aprovar pagamento, registrar recebimento, conciliar, aprovar ajuste de orçamento) fica `disabled`, e `telas-financeiro.test.jsx` falha se algum deles for habilitado antes do backend existir.

> **TI é o mais delicado**: as telas mostram uptime, SLA, latência e "Operacional" de sistemas que **existem de verdade na operação** (Sankhya, n8n, BigQuery, Supabase, Google Workspace). Não há coleta, ping nem integração de monitoramento neste repositório — um "Operacional" ali não significa que o sistema está no ar.

> **Marketing também pede cuidado**: alcance, engajamento, leads e ROAS costumam virar slide de reunião, e **não existe integração com Meta, Google, LinkedIn ou YouTube neste repositório** — nenhum desses números saiu de plataforma nenhuma.

> **Trocas pede cuidado extra**: as telas mostram valor em reais, alçada de aprovação e crédito liberado. Valor inventado que passe por real seria decisão financeira em cima de nada — por isso os botões de aprovar e de liberar crédito ficam `disabled`, e um teste falha se algum deles for habilitado antes do backend.

Os números vêm do `dados-demo.js` de cada módulo e **toda tela exibe `<AvisoDemo modulo="..." />`** dizendo que são de demonstração. Isso não é exceção à regra "nunca mock como substituto de dado real" — a regra proíbe mock que se **disfarça** de dado real; aqui a origem está no nome do arquivo, no aviso da tela e num teste que falha se o aviso sumir (`paginas/telas-*.test.jsx`).

Botão que dependeria de gravar dado fica `disabled` com `title="Depende do backend de …"`, em vez de fingir que funciona.

As peças visuais compartilhadas dessas telas (cabeçalho, KPI pastel, etiqueta, barra de progresso, avatar e o próprio aviso) moram em `src/shared/ui/PecasPainel.jsx` — em `shared/`, e não dentro de um dos módulos, porque um módulo não pode importar do outro.

**Para trocar pelo dado real:** criar as RPCs com `fn_exigir_acesso_modulo('<modulo>')`, expor em `dataAccess.<modulo>.*`, trocar o import de `dados-demo.js` por `useEffect` com os quatro estados (`loading | success | empty | error`) como nas telas de Compras, e aí apagar `dados-demo.js` e o `<AvisoDemo />`. As chaves de `modulos/<nome>/nav.js` entram no `CATALOGO` no mesmo momento.

**Onboarding por tela:** cada página chama `useTourAutomatico(<nome>Tour)`, que abre o guia na primeira visita **e** registra o guia da tela atual. O botão "Ajuda" no rodapé da sidebar lê esse registro — ele é irmão da página na árvore do React, então não dá para passar por props (mesma solução de `useSkylerContext`).
...
Todo tráfego remoto passa por `src/shared/data/data-access.js`, exposto como `dataAccess.{dashboard,auth,users,permissions,parameters}`. Falhas viram `DataAccessError` (`operation`, `code`, `status`, `details`, `hint`).

Proibido em páginas/componentes/contexts/hooks: importar `src/shared/data/supabase-client.js`, ou chamar `.rpc()`, `.from()`, `supabase.auth`. Operação nova entra na camada e é consumida por método semântico.
...
Fonte executável: `supabase/migrations/`, aplicada inteira e em ordem lexicográfica. `supabase/sql/` é histórico — não executar. Smoke tests em `supabase/tests/` (05 = segurança multiempresa, 06 = ETL horário, 07 = compatibilidade de perfil legado, 09 = RPCs de widget, recorte por gráfico e delegação).
...
Em caso de divergência, a ordem de verdade é: `supabase/migrations/` → `supabase/tests/` → `docs/ai/backend.md` → `docs/architecture/supabase_arquitetura.md` → `src/shared/data/data-access.js` e `src/modulos/compras/paginas/*.jsx` → arquivos históricos.
...
**Wrapper `_completo` (desde `20260829120000`).** As 19 RPCs consumidas pelo frontend (9 de tela + 10 de widget) são wrappers finos: `get_budget` chama `get_budget_completo` e aplica `fn_filtrar_blocos_permitidos`, que remove do payload os blocos que o usuário não pode ver. O `_completo` guarda o corpo real e **não tem `EXECUTE` para `authenticated`** — é o que impede pular o recorte chamando-o direto.

Consequência prática: `create or replace function public.get_budget(jsonb)` com o corpo inteiro **apaga o recorte em silêncio**. Mexeu no corpo de uma RPC de tela? Mexa no `_completo`. `npm run check:security` falha se algum dos 9 wrappers perder a chamada de `fn_filtrar_blocos_permitidos`.

A granularidade por gráfico vive em `permissoes_graficos_mapa` (chave do `CATALOGO` → caminho no payload); um caminho compartilhado, como `itens`, só sai quando **todos** os gráficos que o consomem estão negados. Ver `docs/security-audit/mapa-graficos-payload.md`.

### Skyler (chat) e Cloudflare Functions

`src/plataforma/skyler/Skyler.jsx` chama `VITE_SKYLER_API_URL` (rota **relativa**, padrão `/api/skyler/chat`). O proxy é `functions/api/skyler/chat.js`: valida same-origin, valida o Bearer do Supabase em `/auth/v1/user`, limita payload a 64 KB, e só então repassa para `SKYLER_UPSTREAM_URL` com os headers de `.dev.vars`. Nenhum segredo de IA no frontend, e o token de sessão nunca sai para origem externa — os validadores bloqueiam URL absoluta.
...
0. **Estrutura por módulo, com dependência de mão única** — `app → modulos → shared`; `shared/` não conhece módulo nenhum, e módulo novo é pasta nova (só as rotas ficam fora dela). `plataforma/` é a exceção declarada: o painel de debug observa o app inteiro.
...
Tailwind v4 + daisyUI, tema `compras` definido em `src/app/index.css` (único lugar de CSS). Usar classes utilitárias/daisyUI direto no JSX — sem `<style>`, sem arquivo CSS novo. Tokens fora do daisyUI (`--color-orange`, `--color-purple`, `--color-teal`, `--color-critical`) vivem no `:root` do mesmo arquivo e são lidos por `src/shared/lib/theme-color.js` para o canvas dos gráficos. Ícones: `lucide-react`, importados no componente que usa.
...
13. **Devolver `EXECUTE` a uma RPC para destravar um gráfico vazio reabre o modelo de segurança** — é o inverso do problema nº 1 e foi o que causou o achado crítico da auditoria de 29/08/2026: três migrations regrantaram 10 helpers que não têm `fn_exigir_acesso_modulo` nem escopo de empresa, e uma delas ainda os tornou `security definer` **sem trocar o owner** — `security definer` só continua sob RLS se a função pertencer a `compras_rpc_owner` (`nobypassrls`); com o dono da migration, que é dono das tabelas, o RLS é ignorado, porque nenhuma tabela usa `force row level security`. Antes de conceder `EXECUTE` a `authenticated`, confira guarda de módulo **e** owner. `npm run check:security` agora deriva essa lista de todas as migrations, em vez de conferir nomes fixos.
14. **Desativar perfil (`ativo = false`) não revoga a sessão no Supabase Auth** — o token de quem foi desligado continua válido e renovável até expirar. Todo predicado de autorização precisa exigir `ativo = true` (foi assim que um delegado desativado seguia editando permissões), e o `AuthProvider` encerra a sessão ao carregar perfil inativo.
